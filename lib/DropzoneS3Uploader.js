...
var DropzoneS3Uploader = (function (_React$Component) {
  ...
  function DropzoneS3Uploader(props) {
    ...
    this.handleError = function (err, file) {
      ...
      var waitingQueue = _this.state.waitingQueue;

      //swap videos to waiting queue on error
      waitingQueue.shift();

      if (waitingQueue.length > 0) {
        var nextElement = waitingQueue[0];
    ...
    this.handleDrop = function (files, rejectedFiles) {
      var getWaitingQueue = _this.props.getWaitingQueue;
      var waitingQueue = _this.state.waitingQueue;
      ...
      _this.setState({ waitingQueue: [].concat(waitingQueue) });
  ...
  DropzoneS3Uploader.prototype.render = function render() {
    ...
    var getWaitingQueue = _props.getWaitingQueue;

    var dropzoneProps = _objectWithoutProperties(_props, ['s3Url', 'passChildrenProps', 'children', 'imageComponent', 'fileComponent', 'progressComponent', 'errorComponent', 'getWaitingQueue']);
